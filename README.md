...
- 동시성 테스트를 통해 5건 요청 중 3건만 성공하도록 검증

## 일관성

- 주문 생성과 재고 차감을 하나의 트랜잭션으로 묶음
- 중간 실패 시 전체 롤백
- 주문과 재고 상태가 항상 일관되도록 보장

## 멱등성
- 주문과 재고 상태가 항상 일관되도록 보장
- 좋아요 API는 동일 요청 반복 시 상태가 변하지 않도록 설계
- 현재는 명시적 idempotency key는 없지만, 구조상 추후 Idempotency-Key 헤더로 확장 가능

## 느린 조회
- 상품 목록 조회에 페이지네이션(`page`, `size`)과 정렬(`latest`, `price_asc`, `likes_desc`)을 적용
- 정렬 기준 컬럼(`created_at`, `price`, `like_count`)에 인덱스를 적용하여 조회 성능을 개선
- 동일한 조회 조건에 대해서는 Spring Cache를 적용하여 반복 조회 감소
- 상품 등록, 좋아요 등록/취소, 주문 생성 시 상품 목록 캐시를 무효화하여 데이터 최신성을 유지

## 동시 주문
- 동시에 여러 주문 요청이 들어와도 재고 차감이 정확히 이루어지도록 처리
- 비관적 락을 통해 동일 상품에 대한 경쟁 상태를 제어
- 재고 부족 시 예외 처리로 초과 주문 방지
...
```md
활용 내용:
- Spring Boot 계층 구조 및 패키지 설계 보조
- DTO / Service / Repository 코드 초안 작성 보조
- 테스트 코드 및 동시성 테스트 시나리오 설계 보조

활용 후 직접 수정/검증한 내용:
- API 명세에 맞는 요청/응답 구조 수정
- 예외 메시지 및 상태코드 검증
- 동시성 테스트 실패 원인 분석 후 테스트 정리 방식 수정
```